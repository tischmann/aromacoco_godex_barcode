...
const labelPrinter = (function () {
    ...
    function renderSequence() {
        ...
        if (sequence.size) {
            sequenceItemsWrapper.setAttribute(
                "data-height",
                sequenceItemsWrapper.style.height
            );
        ...
        for (var [key, value] of sequence) {
            const sequenceItemPromise = new Promise((resolve, reject) => {
            ...
            sequenceItemPromise.then((element) => {
    ...
    function centerLineMargin(text, lineLength = 15) {
        ...
        let TEXT_WIDTH = text.length * LETTER_WIDTH;
        return LEFT + Math.round((WIDTH - TEXT_WIDTH) / 2);
    }
    ...
    function printSequenceHandler(event) {
        ...
        for (const [key, value] of sequence.entries()) {
            ...
            let BARCODE = TEXT_ONLY ? '' : (DPI_300
                ? `BE,22,90,3,6,32,0,1,${value.barcode}${CR}`
                : `BE,12,49,2,6,32,0,1,${value.barcode}${CR}`);

            let TEXT = TEXT_ONLY
                ? createMultipleLines(value.text)
                : (DPI_300 ? `AJ,${margin(value.code)},${top},2,2,0,0,${value.code}${CR}` : `AI,${margin(value.code)},${top},1,1,0,0,${value.code}${CR}`);
    ...
    function createMultipleLines(text, maxLength = 14) {
        ...
        const MAX_LINES = DPI_300 ? 6 : 4;

        let skipLines = (MAX_LINES - lines.length);

        if (skipLines > 0) {
            textTop += skipLines * textStep / 2;
        }

        lines.forEach((line, idx) => {
            let y = Math.floor(textTop + idx * textStep);
            commands += DPI_300
                ? `AJ,${centerLineMargin(line)},${y},1,1,0,0,${line}${CR}`
                : `AI,${centerLineMargin(line)},${y},1,1,0,0,${line}${CR}`;
        });
